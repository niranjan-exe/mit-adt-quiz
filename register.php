...
$first = $last = $email = $adhar = $enroll = $roll = $password = $confirm_password = "";
$first_err = $last_err = $email_err = $adhar_err = $enroll_errl = $roll_err = $password_err = $confirm_password_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["adhar"]))){
        $email_err = "Please enter your Adhar Card Number.";
    } else{
     
        $sql = "SELECT id FROM users WHERE adhar = ?";
        
        if($stmt = $mysqli->prepare($sql)){
            
            $stmt->bind_param("s", $param_adhar);
            
           
            $param_email = trim($_POST["adhar"]);
            ...
            if($stmt->execute()){
                ...
                if($stmt->num_rows == 1){
                    $eadhar_err = "This adhar is already in use.";
                } else{
    ...
    if(empty($first_err) && empty($last_err) && empty($email_err) && empty($enroll_err) && empty($roll_err) && empty($adhar_err) && empty($password_err) && empty($confirm_password_err)){
        
        
        $sql = "INSERT INTO users (first, last, email, enroll, roll, adhar, password) VALUES (?, ?, ?, ?, ?, ?, ?)";
         
        if($stmt = $mysqli->prepare($sql)){
         
            $stmt->bind_param("sssssss", $param_first, $param_last, $param_email, $param_enroll, $param_roll, $param_adhar, $param_password);
    ...
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" name="adhar" class="form-control <?php echo (!empty($adhar_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $adhar; ?>">
                <span class="invalid-feedback"><?php echo $adhar_err; ?></span>
            </div>
            <div class="form-group">
